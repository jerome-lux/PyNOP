...
class SlicedTimeSeriesDataset(Dataset):
    def __init__(
        ...
        for series_filepath in all_series_files:
            try:
                ...
                with np.load(series_filepath, mmap_mode="r") as data:
                    ...
                    if not first_valid_file_inspected:
                        ...
                        sample_slice = data[self.t_dim]  # This is [H,W] or [C,H,W]
                        ...
                        if self._raw_slice_ndim == 2:  # Raw slice is [H, W]
                            ...
                            self._target_channels = (
                                self._raw_slice_channels
                            )  # For 3D slices, use their own channel count
                        else:
                            raise ValueError(
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            with np.load(filepath, mmap_mode="r") as data_array_mmap:
