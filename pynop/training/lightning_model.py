...
class Model(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.train_config.start_autoregressive is not None and self.train_config.final_autoregressive is not None:
            if epoch < self.train_config.start_autoregressive:
                progress = 0.0
            elif epoch >= self.train_config.final_autoregressive:
                progress = 1.0
            else:
                progress = (epoch - self.train_config.start_autoregressive) / (
                    self.train_config.final_autoregressive - self.train_config.start_autoregressive
                )

            threshold = (T_unroll - 1) - int(progress * max_AR_steps)
        else:
            threshold = T_unroll - 1
        ...
        self.log("loss", torch.sqrt(loss), on_step=True, on_epoch=True, prog_bar=True, logger=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log("val_loss", torch.sqrt(loss), on_step=True, on_epoch=True, prog_bar=True, logger=True)
